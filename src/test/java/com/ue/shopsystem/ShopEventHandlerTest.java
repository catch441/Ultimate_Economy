...
public class ShopEventHandlerTest {
	...
	public void handleInventoryClickTestPlayershopLeftClick() {
		try {
			...
			assertNull(player.nextMessage());
			PlayershopController.deletePlayerShop(PlayershopController.getPlayerShops().get(0));		} catch (ShopSystemException | GeneralEconomyException | PlayerException | TownSystemException e) {
			fail();
		}
	}
