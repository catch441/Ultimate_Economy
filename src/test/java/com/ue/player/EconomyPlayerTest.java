...
public class EconomyPlayerTest {
	...
	try {
	    ...
	    assertEquals("10", String.valueOf(player.getScoreboard().getObjective("bank").getScore(ChatColor.GOLD + "$").getScore()));
	} catch (PlayerException | GeneralEconomyException e) {
	    assertTrue(false);
	}
    }
	...
	try {
	    ...
	    assertEquals(1,player.getScoreboard().getObjectives().size());
	    assertEquals("5", String.valueOf(player.getScoreboard().getObjective("bank").getScore(ChatColor.GOLD + "$").getScore()));
	    ecoPlayer.decreasePlayerAmount(5,false);
	} catch (PlayerException | GeneralEconomyException e) {
	    assertTrue(false);
	}
	...
	try {
	    ...
	    assertEquals("10", String.valueOf(recieverPlayer.getScoreboard().getObjective("bank").getScore(ChatColor.GOLD + "$").getScore()));
	    assertEquals(1,player.getScoreboard().getObjectives().size());
	    assertEquals("0", String.valueOf(player.getScoreboard().getObjective("bank").getScore(ChatColor.GOLD + "$").getScore()));
	} catch (PlayerException | GeneralEconomyException e) {
	    assertTrue(false);
	}
    }
	...
	try {
	    ...
	    assertTrue(ecoPlayer.isScoreBoardDisabled());
	    // check savefile
	    File saveFile1 = EconomyPlayerController.getPlayerFile();
