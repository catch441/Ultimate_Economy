...
public class JobTabCompleter implements TabCompleter {
	...
	private List<String> handleJobTabComplete(String[] args) {
		// If you just switch args[1] then while typing /jobcenter job it will
		// throw an ArrayIndexOutOfBounds
		// during the "job" part. Catch the exception and return a blank list
		// then it won't throw stack traces to console.
		try {
			switch (args[1]) {
			case "":
