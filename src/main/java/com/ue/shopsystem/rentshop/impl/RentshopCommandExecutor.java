...
public class RentshopCommandExecutor implements CommandExecutor {
    ...
    /**
     * Constructor of rentshop command executor.
     * @param plugin
     */
    public RentshopCommandExecutor(UltimateEconomy plugin) {
	this.plugin = plugin;
    }
	...
	if (sender instanceof Player) {
		...
		if (args.length != 0) {
			...
			if (args.length == 3) {
			    RentshopImpl shop = RentshopController.createRentShop(plugin.getDataFolder(),
				    player.getLocation(), Integer.valueOf(args[1]), Double.valueOf(args[2]));
			...
			if (args.length == 3) {
			    RentshopController.getRentShopByUniqueName(args[1])
