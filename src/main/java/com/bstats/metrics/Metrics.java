...
public class Metrics {
	...
	public Metrics(Plugin plugin) {
		...
		if (!config.isSet("serverUuid")) {
			setupConfigFile(configFile, config);
		}
		loadData(config);
		if (enabled) {
			boolean found = false;
	...
	private void setupConfigFile(File configFile, YamlConfiguration config) {
		// Add default values
	...
	private void loadData(YamlConfiguration config) {
	...
	public JSONObject getPluginData() {
		...
		for (AbstractCustomChart abstractCustomChart : charts) {
			// Add the data of the custom charts
			JSONObject chart = abstractCustomChart.getRequestJsonObject();
	...
	private JSONObject getServerData() {
		...
		try {
			...
			if (onlinePlayersMethod.getReturnType().equals(Collection.class)) {
				playerAmount = ((Collection<?>) onlinePlayersMethod.invoke(Bukkit.getServer())).size();
			} else {
				playerAmount = ((Player[]) onlinePlayersMethod.invoke(Bukkit.getServer())).length;
			}
		} catch (Exception e) {
	...
	private static void sendData(Plugin plugin, JSONObject data) throws Exception {
		...
		addHeaders(connection, compressedData);
		// Send data
	...
	private static void addHeaders(HttpsURLConnection connection, byte[] compressedData) throws ProtocolException {
