...
public class PropertyMap extends ForwardingMultimap<String, Property> {
		...
		for (final Map.Entry<String, JsonElement> entry : object.entrySet()) {
			...
			for (final JsonElement element : (JsonArray) entry.getValue()) {
			    result.put(entry.getKey(),
				    new Property(entry.getKey(), element.getAsString()));
			}
		    }
		}
		...
		for (final JsonElement element2 : (JsonArray) json) {
			...
			if (object2.has("signature")) {
			    result.put(name, new Property(name, value,
				    object2.getAsJsonPrimitive("signature").getAsString()));
			} else {
			    result.put(name, new Property(name, value));
			}
		    }
	...
	@Override
	public JsonElement serialize(final PropertyMap src, final Type typeOfSrc,
